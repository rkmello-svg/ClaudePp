...
@Injectable()
export class IngenicProvider implements ITefProvider {
  name = 'ingenico';
  enabled = false;
  private readonly logger = new Logger(IngenicProvider.name);
  private config: TefProviderConfig;

  constructor(config?: TefProviderConfig) {
    this.config = config || {};
    this.logger.log('Ingenico TEF provider initialized (stub)');
  }
  ...
  async processCardPayment(request: CardPaymentRequest): Promise<PaymentResponse> {
    ...
    // TODO: Implement Ingenico SDK integration
    return {
      transactionId: `INGENICO_${Date.now()}`,
      authCode: '789012',
      status: 'approved',
  ...
  async processPixPayment(request: PixPaymentRequest): Promise<PaymentResponse> {
    ...
    // TODO: Implement Ingenico SDK integration
    return {
      transactionId: `INGENICO_PIX_${Date.now()}`,
      status: 'pending',
      amount: request.amount,
      timestamp: new Date(),
    };
  ...
  async processCashPayment(request: PaymentRequest): Promise<PaymentResponse> {
    ...
    return {
      transactionId: `INGENICO_CASH_${Date.now()}`,
      authCode: 'CASH',
      status: 'approved',
      amount: request.amount,
      timestamp: new Date(),
  ...
  async refund(transactionId: string, amount: number): Promise<PaymentResponse> {
    ...
    // TODO: Implement Ingenico SDK integration
    return {
      transactionId: `INGENICO_REFUND_${Date.now()}`,
      status: 'approved',
      amount,
      timestamp: new Date(),
    };
  ...
  async testConnection(): Promise<boolean> {
    this.logger.log('Testing Ingenico connection');
