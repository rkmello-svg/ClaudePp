...
@Injectable()
export class StoneProvider implements ITefProvider {
  name = 'stone';
  enabled = false;
  ...
  constructor(config?: TefProviderConfig) {
    this.config = config || {};
    this.logger.log('Stone TEF provider initialized (stub)');
  }
  ...
  async processCardPayment(request: CardPaymentRequest): Promise<PaymentResponse> {
    ...
    // TODO: Implement Stone SDK integration
  ...
  async processPixPayment(request: PixPaymentRequest): Promise<PaymentResponse> {
    ...
    // TODO: Implement Stone SDK integration
    return {
      transactionId: `STONE_PIX_${Date.now()}`,
      status: 'pending',
      amount: request.amount,
      timestamp: new Date(),
    };
  ...
  async processCashPayment(request: PaymentRequest): Promise<PaymentResponse> {
    ...
    return {
      transactionId: `STONE_CASH_${Date.now()}`,
      authCode: 'CASH',
      status: 'approved',
      amount: request.amount,
      timestamp: new Date(),
  ...
  async refund(transactionId: string, amount: number): Promise<PaymentResponse> {
    ...
    // TODO: Implement Stone SDK integration
    return {
      transactionId: `STONE_REFUND_${Date.now()}`,
      status: 'approved',
      amount,
      timestamp: new Date(),
    };
  ...
  async testConnection(): Promise<boolean> {
    this.logger.log('Testing Stone connection');
